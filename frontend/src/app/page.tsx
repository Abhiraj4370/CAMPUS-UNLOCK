...
export default function HomePage() {
  ...
  useEffect(() => {
    api.get<{ items: Category[] }>('/universities/categories/all').then((d) => setCategories(d.items)).catch(() => setCategories([]));
    api.get<{ items: University[] }>('/universities/featured').then((d) => setFeatured(d.items)).catch(() => setFeatured([]));
    api.get<{ items: University[]; total: number }>('/universities', { limit: 1 }).then((d) => setTotalUniversities(d.total || 200)).catch(() => { });
  }, []);
  ...
  return (
    <>
      ...
      <main>
        ...
        {featured.length > 0 && (
          <section className="max-w-[1260px] mx-auto px-6 pb-14 overflow-hidden relative">
